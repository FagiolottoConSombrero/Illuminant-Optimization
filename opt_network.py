...
class IllNetwork(pl.LightningModule):
    def __init__(
        ...
        # Ottimizza solo gli illuminanti
        self.ill_optimizer = IlluminantOptimizerL(
            num_illuminants=self.n_ill,
            led_path=self.led_path
        )

    def forward(self, x):
        """
        x: HSI reference [B, C, H, W]

        Returns:
            ills : optimized illuminants
            rgb1 : rendered RGB under illuminant 1
            rgb2 : rendered RGB under illuminant 2
        """

        # 1. Generate optimized illuminants
        ills = self.ill_optimizer()

        # 2. Render RGB images from HSI
        rgb1, rgb2 = render_rgb(
            x,
            ills,
            self.camera_spd_path
        )
    ...
    def step(self, batch, stage):
        ...
        loss_illum, illum_logs = illumination_spec_regularization(ills)

        # RGB image regularization
        loss_img, img_logs = illumination_img_regularization(rgb1, rgb2)

        # Total loss: only rendering / illuminant optimization
        loss = self.w_illum * loss_illum + loss_img
    ...
    def validation_step(self, batch, batch_idx):
        loss, ills, rgb1, rgb2 = self.step(batch, "val")
